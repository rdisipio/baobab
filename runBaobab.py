...
"""
  ...
  try:
    analysis_plugin = imp.load_source( args.analysis, args.analysis + ".py" )
  except:
  ...
  if doTruth:
    ...
    tree_particle = TChain( "particleLevel", "Particle Level" )
  ...
  for fname in open( filelistname, 'r' ).readlines():
    ...
    tree_reco.Add( fname )
    if doTruth: 
      tree_parton.Add( fname )
      tree_particle.Add( fname )

  np_tree_reco = tree2rec( tree_reco )
  ...
  if doTruth:
     np_tree_parton   = tree2rec( tree_parton )
     np_tree_particle = tree2rec( tree_particle )
